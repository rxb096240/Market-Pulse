...
let latestStockData = {};
let currentView = 'crypto-watchlist';
...
function fmtCap(v){
  if(v >= 1e12) return '$'+(v/1e12).toFixed(2)+'T mcap';
  if(v >= 1e9) return '$'+(v/1e9).toFixed(2)+'B mcap';
  if(v >= 1e6) return '$'+(v/1e6).toFixed(2)+'M mcap';
...
async function fetchViaProxies(target){
  // Try a direct request first, then fall back through a few CORS proxies —
  // any single one of these can be flaky or rate-limited on its own.
  const sources = [
    target,
  ...
  for(let i = 0; i < sources.length; i++){
    try{
      return await fetchJsonWithTimeout(sources[i], 8000);
    }catch(e){
      lastErr = e;
      if(i < sources.length - 1) await sleep(150);
      continue;
    }
  }
  throw lastErr || new Error('all sources failed for ' + target);
}

async function fetchOneStock(sym){
  // Cache-bust so intermediate CORS proxies can't serve a stale cached response
  const target = `https://query1.finance.yahoo.com/v8/finance/chart/${sym}?interval=1d&range=1d&_=${Date.now()}`;
...
async function fetchStocks(){
  ...
  for(let i = 0; i < STOCKS.length; i++){
    ...
    // Stagger requests so we don't burst the shared free CORS proxies all at once
...
async function fetchTrending(){
  try{
    const json = await fetchJsonWithTimeout('https://api.coingecko.com/api/v3/search/trending', 8000);
    return json.coins || [];
  }catch(e){
...
async function fetchCryptoNewsFor(coin){
  ...
  const target = `https://query1.finance.yahoo.com/v1/finance/search?q=${encodeURIComponent(yahooSym)}&newsCount=4&quotesCount=0`;
  try{
    const json = await fetchViaProxies(target);
...
async function fetchCryptoNews(){
  ...
  for(let i = 0; i < COINS.length; i++){
    results.push(await fetchCryptoNewsFor(COINS[i]));
...
async function fetchStockNewsFor(sym){
  const target = `https://query1.finance.yahoo.com/v1/finance/search?q=${encodeURIComponent(sym)}&newsCount=4&quotesCount=0`;
  try{
    const json = await fetchViaProxies(target);
    const items = json.news || [];
    return items.slice(0, 4).map(n => ({
...
async function fetchAllStockNews(){
  const results = [];
  for(let i = 0; i < STOCKS.length; i++){
    results.push(await fetchStockNewsFor(STOCKS[i].sym));
    if(i < STOCKS.length - 1) await sleep(300);
  }
  return results.flat();
}
...
initGrids();
trackSavedPortfolioAssets().then(() => { refreshAll(); refreshNews(); });
renderPortfolio();
tickClock();
