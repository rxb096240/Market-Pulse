...
//
// The frontend used to call CoinGecko, Yahoo Finance, and Google News RSS
// directly from the browser. CoinGecko allows CORS, but Yahoo Finance and
// Google News do not — that's why the old script.js raced requests through
// third-party public CORS proxies (allorigins.win, codetabs.com). Those are
// unreliable and rate-limited.
//
// This server does the fetching itself (server-to-server calls have no CORS
// restrictions) and exposes clean same-origin JSON/XML endpoints for the
// frontend to call instead. It also serves the static frontend files.
...
app.get('/api/news/google', async (req, res) => {
  ...
  if (!url) return res.status(400).json({ error: 'edition must be "us" or "world"' });
  try {
